  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', background: '#FFFCF7', fontFamily: 'Heebo, sans-serif', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 480, margin: '0 auto', padding: '40px 24px 80px' }}>
        <button
          onClick={() => navigate('/')}
          style={{ cursor: 'pointer', background: 'transparent', border: '2px solid #EBE3D6', borderRadius: 999, padding: '8px 16px', fontFamily: 'Heebo, sans-serif', fontWeight: 600, fontSize: 14, color: '#4A4453', marginBottom: 32 }}
        >
          חזרה לחנות ←
        </button>
        ...
        <div style={{ background: '#fff', border: '3px solid #16121F', borderRadius: 24, boxShadow: SH(6, 6, 0, '#16121F'), overflow: 'hidden' }}>
          ...
          <div style={{ background: '#E6DBFF', borderBottom: '3px solid #16121F', padding: '28px 28px 24px', display: 'flex', alignItems: 'center', gap: 18 }}>
            ...
            <div>
              <div style={{ fontFamily: 'Rubik, sans-serif', fontWeight: 700, fontSize: 22, lineHeight: 1.2 }}>
                {user.displayName ?? 'קונה'}
              </div>
              <div style={{ fontSize: 14, color: '#6B6475', marginTop: 4 }}>{user.email}</div>
              {createdAt && (
                <div style={{ fontSize: 12, color: '#8A8194', marginTop: 2 }}>חברה מ{createdAt}</div>
              )}
            </div>
          </div>
          ...
          <div style={{ padding: '24px 28px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <input
                ...
                style={{ flex: 1, padding: '11px 14px', border: '2.5px solid #16121F', borderRadius: 12, fontFamily: 'Heebo, sans-serif', fontSize: 15, background: '#fff', outline: 'none' }}
              />
              <button
                onClick={savePhone}
                disabled={saving}
                className="btn-lift"
                style={{ cursor: saving ? 'wait' : 'pointer', padding: '11px 18px', background: '#FF3D8B', color: '#fff', border: '2.5px solid #16121F', borderRadius: 12, fontFamily: 'Rubik, sans-serif', fontWeight: 700, fontSize: 15, boxShadow: SH(3, 3, 0, '#16121F'), opacity: saving ? 0.65 : 1, transition: 'transform .12s', minWidth: 64 }}
              >
                {savedMsg ? '✓' : saving ? '…' : 'שמור'}
              </button>
            </div>
            <p style={{ margin: '8px 0 0', fontSize: 12, color: '#8A8194' }}>משמש לעדכוני הזמנות בלבד.</p>
          </div>
          ...
          <div style={{ padding: '0 28px 24px' }}>
            ...
            <button
              ...
              className="btn-lift"
              style={{ cursor: 'pointer', width: '100%', padding: '12px 20px', background: '#fff', color: '#16121F', border: '2.5px solid #16121F', borderRadius: 14, fontFamily: 'Heebo, sans-serif', fontWeight: 700, fontSize: 15, boxShadow: SH(3, 3, 0, '#16121F'), transition: 'transform .12s' }}
            >
              התנתק
            </button>
          </div>
